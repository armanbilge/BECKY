...
public class SimpleCophylogenyModelTest {
    ...
    private CophylogenyLikelihood createCophylogenyLikelihood(final Tree symbiont) {
		return new CophylogenyLikelihood(host, (MutableTree) symbiont, model, branchRates, origin, "host.nodeRef", "testCophylogenyLikelihood");
	}
...
//	    final double actualLikelihood = 0.0 + Math.exp(0.0 * overallRate) * (1 - Math.exp(3 * lossRate)) * ((1 - Math.exp(2 * lossRate) + Math.exp(2 * overallRate) * (1 - Math.exp(1 * lossRate) * (1 - Math.exp(1 * lossRate)))));
	...
	public void testCorrectness() {
	    ...
	    CophylogenyLikelihood likelihood = new CophylogenyLikelihood(host, (MutableTree) symbiont, new SimpleCophylogenyModel(new Parameter.Default(1), new Parameter.Default(1), new Parameter.Default(1), Units.Type.YEARS), new SimpleBranchRates(1.0), new Parameter.Default(1), "host.nodeRef", "ActualCophylogenyLikelihood");
	    ...
	    TestUtils.reduceDecimalPrecision((MutableTree) host); TestUtils.reduceDecimalPrecision((MutableTree) symbiont);
	    likelihood = new CophylogenyLikelihood(host, (MutableTree) symbiont, new SimpleCophylogenyModel(new Parameter.Default(1.8184E-9), new Parameter.Default(111.1071), new Parameter.Default(1.0), Units.Type.YEARS), new SimpleBranchRates(5.1416E-2), new Parameter.Default(1), "host.nodeRef", "EstimatedCophylogenyLikelihood");
